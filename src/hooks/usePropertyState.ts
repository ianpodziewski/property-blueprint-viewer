...
export interface BuildingSummary {
  ...
  floorTemplateBreakdown: Record<string, number>;
  warnings: BuildingWarning[];
}

// Interface for building warnings
export interface BuildingWarning {
  type: 'overallocated' | 'underutilized' | 'excessiveUnits';
  floorIds?: string[];
  unitTypeId?: string;
  message: string;
...
export const usePropertyState = () => {
  ...
  const calculateBuildingSummary = (unitAllocations: Record<string, Record<string, number>> = {}): BuildingSummary => {
    // Total floors
